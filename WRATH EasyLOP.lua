...
StaticPopupDialogs["CAPTURE_DEORGREED"] = {
  ...
  OnAccept = function()
	if bSendToGreed == false then
		EasyLOP:setDE(getglobal(this:GetParent():GetName().."EditBox"):GetText())
		EasyLOP:ELOP_FrameRefresh()
	else
		EasyLOP:setGreed(getglobal(this:GetParent():GetName().."EditBox"):GetText())
		EasyLOP:ELOP_FrameRefresh()
	end
	...
	for i = 1, numLoots, 1 do	
		if LootSlotIsItem(i) then
			...
			if iRarity >= iRarityThreshold then
				if sName == "Badge of Justice" or sName == "Emblem of Valor" or sName == "Emblem of Heroism" or sName == "Emblem of Conquest" or sName == "Emblem of Triumph" or sName == "Ashen Sack of Gems" or sName == "Emblem of Frost" then
					--local lootIcon, lootName, lootQuantity, rarity = GetLootSlotInfo(i)
					--LootSlot(i)
					--iTotalBadges = iTotalBadges + lootQuantity
				elseif sName == "Abyss Crystal" or sName == "Void Crystal" or sName == "Nexus Crystal" then
					local lootIcon, lootName, lootQuantity, rarity = GetLootSlotInfo(i)
					LootSlot(i)
				else
					bMLQualified = true
	...
	if bMLQualified == true then		
		...
		if bAutoAnnounce == true then
			...
			if bOverwrite == true then
				SendAddonMessage("ELOP_SET_SUSPEND", "true " .. UnitName("player"), "RAID")
				...
				SendAddonMessage("ELOP_KILL_POPUPS", "", "RAID")
				
				--self:Print("Overwrite is true.")
				bBidsActive = false
				iCurrentLootIndex = 1
				SendAddonMessage("ELOP_UPDATE_INDEX", iCurrentLootIndex .. " " .. UnitName("player") .. " OpenBids", "RAID")				
				tLoots = {}
				...
				SendAddonMessage("ELOP_CLEAR_TLOOTS", "", "RAID")
				...
				for i, loot in pairs(tTempLoots) do
					...
					SendAddonMessage("ELOP_ADD_TLOOTS_ENTRY", sendString, "RAID")
				...
				SendAddonMessage("ELOP_SHOW_OPENBIDS", strTexture, "RAID")
...
function EasyLOP:OpenBids()
	...
	if tLoots[1] ~= nil and bMLQualified == true then
		...
		if tLoots[iCurrentLootIndex] == nil then
			...
			bBidsActive = true
			SendChatMessage("Send tells (Shroud/Standard/Save) to " .. UnitName("player") .. " for " .. tLoots[iCurrentLootIndex].item .. strQuant, "RAID_WARNING")
			SendAddonMessage("ELOP_HIDE_OPENBIDS", "", "RAID")
...
function EasyLOP:SendWinnerEntry(thisBid)
	...
	SendAddonMessage("ELOP_UPDATE_WINNER_ENTRY", sendString, "RAID")
end
	...
	if tLoots[iCurrentLootIndex].quant == 1 then 
		...
		if numBids < 2 or bOnly1Shroud then
			...
			if numBids == 0 then
				...
				SendAddonMessage("ELOP_SYNCDELOG", sendString, "RAID")
...
function EasyLOP:AcceptAnnouncement()
	...
	for i, winners in pairs(tWinners) do
		if winners.bidder ~= 0 and winners.bidder ~= "" then
			...
			SendAddonMessage("ELOP_SYNCPOINTS", strSync, "RAID")
		end
	end
	...
	if lootSystem == 'master' and tLoots[iCurrentLootIndex].quant == 1 and bAutoAward == true then -- review for automatic distribution to Winner
		...
		if strRecipient ~= 0 then
			...
			if EasyLOP:IsML(UnitName("player")) == false then
				...
				SendAddonMessage("ELOP_SENDTOML", sendString, "RAID")
			else
	...
	SendAddonMessage("ELOP_UPDATE_QUANT", sendString, "RAID")
	...
	-- iCurrentLootIndex .. " " .. UnitName("player") .. " " .. callingFunct
	SendAddonMessage("ELOP_UPDATE_INDEX", iCurrentLootIndex .. " " .. UnitName("player") .. " AcceptAnnouncement", "RAID")
	
	if tLoots[iCurrentLootIndex] ~= nil then
		--StaticPopup_Show ("BEGIN_BIDS", EasyLOP:getItemName(tLoots[iCurrentLootIndex].item))
		SendAddonMessage("ELOP_SHOW_OPENBIDS", tLoots[iCurrentLootIndex].item, "RAID")
	else
		SendAddonMessage("ELOP_SET_SUSPEND", "false " .. UnitName("player"), "RAID")
		tRollItems = {}
	end
end
...
function EasyLOP:LOOT_SLOT_CLEARED(iSlotCleared)
	if bMLQualified == true and LootSlotIsItem(iSlotCleared) then
...
function EasyLOP:PARTY_LOOT_METHOD_CHANGED()
	...
	if lootSystem == 'master' then
		masterLooterName = GetRaidRosterInfo(masterLooterID)
...
function EasyLOP:VerCheck()
	...
	SendAddonMessage("ELOP_VERCHECK", "", "RAID")
end
...
function EasyLOP:CHAT_MSG_LOOT(strContent)
	...
	if bVashjTest == true and GetNumRaidMembers() > 6 then
...
function EasyLOP:CHAT_MSG_RAID_WARNING(msg, author, lang)
	...
	if author == UnitName("player") and (string.find(msg, "*Loots: ") or msgQIndex > 1) then
		...
		if tMsgQ[msgQIndex] == nil then
			...
			--msgQIndex = 1
			SendAddonMessage("ELOP_CLEAR_MSGQ", "", "RAID")
			return
		else
...
function EasyLOP:CHAT_MSG_WHISPER(text, author, lang, status)
	...
	elseif ltext == "?lop?" then
		...
		SendChatMessage('When loot is announced, you can send a tell to the LOOT MASTAH saying "Shroud", "Standard", or "Save".', "WHISPER", nil, author)
...
function EasyLOP:CHAT_MSG_ADDON(prefix, msg, disType, sender)
	...
	if prefix == "ELOP_SENDTOML" then
		...
		return
	elseif prefix == "ELOP_SYNCPOINTS" and bOverrideSync == false then
	...
	elseif prefix == "ELOP_SHOW_OPENBIDS" then
		beginBidsFrame = StaticPopup_Show ("BEGIN_BIDS", msg)  
	elseif prefix == "ELOP_HIDE_OPENBIDS" then
		StaticPopup_Hide("BEGIN_BIDS")
	elseif prefix == "ELOP_SET_SUSPEND" then
	...
	elseif prefix == "ELOP_UPDATE_QUANT" then
		for incindex, incquant, incitem in string.gmatch(msg, "(%d+) (%d+) (.+)") do
	...
	elseif prefix == "ELOP_UPDATE_WINNER_ENTRY" then
		-- insert code
		--[[
		sendString = ""
		sendString = sendString .. iCurrentLootIndex .. " " 
	...
	elseif prefix == "ELOP_CLEAR_MSGQ" then
		tMsgQ = {}
		msgQIndex = 1
		return
	elseif prefix == "ELOP_VERCHECK" then
		...
		SendAddonMessage("ELOP_VERREPORT", s, "RAID")
	elseif prefix == "ELOP_SYNCDELOG" then
		--sendString = UnitName("player") .. " " .. tLoots[iCurrentLootIndex].item
		for incsender, incitem in string.gmatch(msg, "(.+) && (.+)") do
			EasyLOP:LogEvent("Disenchant", "", "", incsender, "", incitem)
		end
	elseif prefix == "ELOP_AUTOCHARGE" then
	...
	elseif prefix == "ELOP_VERREPORT" then
...
function EasyLOP:UserImport( raidtype )
	if raidtype == "10" then
		raidtype = "10-man"
	elseif raidtype == "25" then
		raidtype = "25-man"
	elseif raidtype == "" then
		raidtype = "Cataclysm"
	end
	
	self:Print( "Imported points for " .. EasyLOP:ImportData(raidtype) .. " character names (" .. EasyLOP.db.profile.RaidType .. ").  Time stamp: " .. EasyLOP.db.profile.ExportTime .. ".")
end
...
function EasyLOP:ImportData( pool )
	-- Clear the DB
	EasyLOP:ResetDB("profile")
...
--	for charName, charPoints in pairs(EasyLOP.Points.Import.Points) do
	for charName, charPoints in pairs(LeftoversDB.PointsPools[pool].Points) do
...
--	EasyLOP.db.profile.ExportTime = EasyLOP.Points.Import.ExportTime
	...
	EasyLOP.db.profile.RaidType = pool
	
	EasyLOP:LogEvent("Import", pool, "", UnitName("player"))
	return iCount
end
...
function EasyLOP:ModifyPoints( charName, bid, item)
	...
	if item ~= nil and item ~= "" then
		...
		--for inctype, incbidder, incsender, incbidtype, inccharged, incitem in string.gmatch(msg, "(.+) (.+) (.+) && (.+) && (.+) item: (.+)") do
		--for incindex, incbidder, incbidtype, incpoints, incitem in string.gmatch(msg, "(%d+) (.+) && (.+) && (.+) item: (.+)") do
		SendAddonMessage("ELOP_AUTOCHARGE", sendString, "RAID") -- you were here
	end
