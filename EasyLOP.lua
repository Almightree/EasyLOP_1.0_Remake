...
StaticPopupDialogs["CAPTURE_DEORGREED"] = {
  ...
  OnAccept = function()
	if bSendToGreed == false then
        ...
        local editText = CAPTURE_DEORGREED.editBox:GetText()
...
--	for charName, charPoints in pairs(EasyLOP.Points.Import.Points) do
	for charName, charPoints in pairs(LeftoversDB.PointsPools["Cataclysm"].Points) do
...
--	EasyLOP.db.profile.ExportTime = EasyLOP.Points.Import.ExportTime
	...
	EasyLOP.db.profile.RaidType = "Cataclysm"
	
	EasyLOP:LogEvent("Import", "Cataclysm", "", UnitName("player"))
	return iCount
end
